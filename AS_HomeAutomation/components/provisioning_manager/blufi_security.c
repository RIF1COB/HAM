 ...
 *
 * Vendored from ESP-IDF example examples/bluetooth/blufi/main/blufi_security.c.
 * Provides the DH-key-negotiation + AES-CFB128 + CRC16 callbacks required by
 * the BluFi profile. Public symbols are declared in blufi_internal.h.
 */
...
void blufi_dh_negotiate_data_handler(uint8_t *data, int len, uint8_t **output_data, int *output_len, bool *need_free)
{
    ...
    case SEC_TYPE_DH_PARAM_DATA: {
        ...
        if (len < (blufi_sec->dh_param_len + 1)) {
            BLUFI_ERROR("%s, invalid dh param len", __func__);
            btc_blufi_report_error(ESP_BLUFI_DH_PARAM_ERROR);
            return;
        }
        ...
        if (dhm_len > DH_SELF_PUB_KEY_LEN) {
            BLUFI_ERROR("%s dhm len not support %d", __func__, dhm_len);
            btc_blufi_report_error(ESP_BLUFI_DH_PARAM_ERROR);
            return;
        }
        ...
        ret = mbedtls_dhm_calc_secret(&blufi_sec->dhm,
                                      ...
                                      &blufi_sec->share_len, myrand, NULL);
        if (ret) {
...
int blufi_aes_encrypt(uint8_t iv8, uint8_t *crypt_data, int crypt_len)
{
    ...
    iv0[0] = iv8;
    int ret = mbedtls_aes_crypt_cfb128(&blufi_sec->aes, MBEDTLS_AES_ENCRYPT, crypt_len, &iv_offset, iv0, crypt_data, crypt_data);
...
int blufi_aes_decrypt(uint8_t iv8, uint8_t *crypt_data, int crypt_len)
{
    ...
    memcpy(iv0, blufi_sec->iv, sizeof(blufi_sec->iv));
    iv0[0] = iv8;
    int ret = mbedtls_aes_crypt_cfb128(&blufi_sec->aes, MBEDTLS_AES_DECRYPT, crypt_len, &iv_offset, iv0, crypt_data, crypt_data);
    return ret ? -1 : crypt_len;
}
